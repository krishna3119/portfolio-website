...
export default function ContactSection() {
    ...
    const handleNext = () => {
        ...
        if (step === 2 && !form.interest) return;
        setStep(step + 1);
    };
    ...
    const handleSend = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            ...
            // Data mapping matching your precise EmailJS template variables
        ...
        <section id="contact" className="relative py-28 md:py-36 px-6">
            <div className="max-w-4xl mx-auto">
                ...
                <motion.div
                    ...
                    className="glass-card p-8 md:p-14 min-h-[400px] flex flex-col justify-center relative overflow-hidden"
                >
                    <AnimatePresence mode="wait">
                        ...
                        {step === 1 && (
                            ...
                            >
                                <h3 className="text-3xl md:text-4xl lg:text-5xl leading-[1.8] md:leading-[1.8] text-text-primary" style={{ fontFamily: "var(--font-family-display)" }}>
                                    <span className="whitespace-nowrap">Hi, my name is</span>{" "}
                                    <input 
                                        type="text" 
                                        value={form.name} 
                                        onChange={(e) => setForm({ ...form, name: e.target.value })} 
                                        className="inline-block bg-transparent border-0 border-b-2 border-neon/50 text-neon focus:outline-none focus:border-neon px-2 mx-1 min-w-[120px] w-[50%] md:w-[250px] placeholder-neon/30 align-baseline transition-colors" 
                                    ...
                                    <input 
                                        ...
                                        className="inline-block bg-transparent border-0 border-b-2 border-neon/50 text-neon focus:outline-none focus:border-neon px-2 mx-1 min-w-[200px] w-[80%] md:w-[350px] placeholder-neon/30 align-baseline transition-colors" 
                        ...
                        {step === 2 && (
                            ...
                            >
                                <h3 className="text-3xl md:text-5xl text-text-primary mb-8" style={{ fontFamily: "var(--font-family-display)" }}>
                                    I&apos;m interested in...
                                </h3>
                                <div className="flex flex-wrap gap-4">
                                    {["Website Development", "Graphic Design", "Social Media / Content"].map((option) => (
                                        <button 
                                            key={option}
                                            onClick={() => setForm({ ...form, interest: option })}
                                            className={`px-6 py-4 rounded-full border-2 text-sm md:text-lg transition-all duration-300 font-medium tracking-wide ${form.interest === option ? 'border-neon bg-neon/10 text-neon shadow-[0_0_15px_rgba(204,153,255,0.4)]' : 'border-white/10 text-text-secondary hover:border-neon/50 hover:text-white'}`}
                                        >
                                            {option}
                                        </button>
                                    ))}
                        ...
                        {step === 3 && (
                            ...
                            >
                                <h3 className="text-3xl md:text-4xl lg:text-5xl leading-[1.8] md:leading-[1.8] text-text-primary flex flex-wrap items-end" style={{ fontFamily: "var(--font-family-display)" }}>
                        ...
                        {step === 4 && (
                            ...
                            >
                                <h3 className="text-4xl md:text-6xl text-neon font-bold mb-2 drop-shadow-[0_0_15px_rgba(204,153,255,0.6)]" style={{ fontFamily: "var(--font-family-display)" }}>
                                    Done!
                                </h3>
                                <p className="text-xl md:text-3xl text-text-primary font-medium tracking-wide">
                                    Will come back 2 U shortly :)
                                </p>
                                <button onClick={resetForm} className="btn-glow mt-8">
                                    CLOSE
                                </button>
                            </motion.div>
                        )}
